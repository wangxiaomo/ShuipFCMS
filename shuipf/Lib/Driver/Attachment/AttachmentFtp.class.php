...
/**
 * 附件，远程FTP存储方案
 * 思路：先上传到本地，在上传到远程FTP，，等于上传两次。。。
 * Some rights reserved：abc3210.com
 * Contact email:admin@abc3210.com
 */
class AttachmentFtp extends AttachmentService {
    ...
    function __construct($options = array()) {
        ...
        $options = array_merge(array(
            //上传用户ID
            'userid' => AppframeAction::$Cache['uid'] ? AppframeAction::$Cache['uid'] : 0,
            //上传用户组
            'groupid' => 8,
            //是否后台
            'isadmin' => 0,
        ...
        import('Ftp');
        $this->handler = get_instance_of('Ftp');
            ...
            foreach ($info as $i => $value) {
                ...
                if ($aid) {
                    ...
                    try {
                        ...
                        Log::write('文件[' . $info[$i]['savepath'] . $info[$i]['savename'] . ']删除失败！');
                    }
                    unset($info[$i]);
                }
            }
    ...
    public function delFile($file) {
        ...
        if (is_int($file)) {
            ...
            if ($info) {
                ...
                if (!D('Attachment')->where(array("aid" => $file))->delete()) {
