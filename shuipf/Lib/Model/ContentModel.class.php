...
class ContentModel extends RelationModel {
    ...
    public function add($data = '', $options = array(), $replace = false) {
        ...
        if ($inputinfo) {
            $inputinfo = $this->create($inputinfo, 1);
            if ($inputinfo) {
                return parent::add($inputinfo, $options, $replace);
            } else {
                $this->tokenRecovery($data);
                return false;
            }
        } else {
            $this->error = $inputinfo->getError();
            return false;
        }
    }
    ...
    public function save($data = '', $options = array()) {
        ...
        if ($inputinfo) {
            $inputinfo = $this->create($inputinfo, 2);
            if ($inputinfo) {
                return parent::save($inputinfo, $options);
            } else {
                $this->tokenRecovery($data);
                return false;
            }
